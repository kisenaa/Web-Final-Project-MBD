...
import { Asdos } from '../../Interface/interface';
...
const handleSubmit = async () => {
  const response = await deleteData('ASDOS', [
    { props: 'asdos_kode', value: props.row.getValue('asdos_kode') },
  ]);
  if (response === 200) {
    const newData = tableState.datas.filter(
      (item: any) => item.asdos_kode !== props.row.getValue('asdos_kode')
    );
  ...
  >
    ...
    <div class="mt-4 overflow-x-auto">
      <table class="table border border-solid border-[#dddddd]">
        <thead>
          <tr>
            <th class="border border-solid border-[#dddddd]">Kode</th>
            <th class="border border-solid border-[#dddddd]">Nama</th>
            <th class="border border-solid border-[#dddddd]">NRP</th>
            <th class="border border-solid border-[#dddddd]">Angkatan</th>
            <th class="border border-solid border-[#dddddd]">No.telp</th>
          </tr>
        </thead>
        <tbody>
          <tr class="border border-solid border-[#dddddd]">
            <td class="border border-solid border-[#dddddd]">
              {{ row.getValue('asdos_kode') }}
            </td>
            <td class="border border-solid border-[#dddddd]">
              {{ row.getValue('asdos_nama') }}
            </td>
            <td class="border border-solid border-[#dddddd]">
              {{ row.getValue('asdos_nrp') }}
            </td>
            <td class="border border-solid border-[#dddddd]">
              {{ row.getValue('asdos_angkatan') }}
            </td>
            <td class="border border-solid border-[#dddddd]">
              {{ row.getValue('asdos_noTelp') }}
            </td>
          </tr>
        </tbody>
      </table>
