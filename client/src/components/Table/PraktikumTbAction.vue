...
import { Praktikum } from '../../Interface/interface';
...
const handleSubmit = async () => {
  const response = await deleteData('PRAKTIKUM', [
    { props: 'prak_kode', value: props.row.getValue('prak_kode') },
  ]);
  if (response === 200) {
    const newData = tableState.datas.filter(
      (item: any) => item.prak_kode !== props.row.getValue('prak_kode')
    );
  ...
  >
    ...
    <div class="mt-4 overflow-x-auto">
      <table class="table border border-solid border-[#dddddd]">
        <thead>
          <tr>
            <th class="border border-solid border-[#dddddd]">Kode Praktikum</th>
            <th class="border border-solid border-[#dddddd]">Judul</th>
            <th class="border border-solid border-[#dddddd]">Tanggal</th>
        ...
        <tbody>
          <tr class="border border-solid border-[#dddddd]">
            <td class="border border-solid border-[#dddddd]">
              {{ row.getValue('prak_kode') }}
            </td>
            <td class="border border-solid border-[#dddddd]">
              {{ row.getValue('prak_judul') }}
            </td>
            <td class="border border-solid border-[#dddddd]">
              {{ row.getValue('prak_tglPraktikum') }}
            </td>
            <td class="border border-solid border-[#dddddd]">
              {{ row.getValue('ruang_kode') }}
            </td>
            <td class="border border-solid border-[#dddddd]">
