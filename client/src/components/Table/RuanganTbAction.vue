...
import { Ruangan } from '../../Interface/interface';
...
const handleSubmit = async () => {
  const response = await deleteData('RUANGAN', [
    { props: 'ruang_kode', value: props.row.getValue('ruang_kode') },
  ]);
  if (response === 200) {
    const newData = tableState.datas.filter(
      (item: any) => item.ruang_kode !== props.row.getValue('ruang_kode')
    );
      ...
      <table class="table border border-solid border-[#dddddd]">
        ...
        <tbody>
          <tr class="border border-solid border-[#dddddd]">
            ...
            <td class="border border-solid border-[#dddddd]">
              {{ row.getValue('ruang_nama') }}
            </td>
            <td class="border border-solid border-[#dddddd]">
              {{ row.getValue('ruang_kapasitas') }}
            </td>
          </tr>
        </tbody>
      </table>
