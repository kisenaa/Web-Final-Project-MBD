...
const handleSubmit = async () => {
  const response = await deleteData('MATA_KULIAH', [
    { props: 'mk_kode', value: props.row.getValue('mk_kode') },
  ]);
  if (response === 200) {
    const newData = tableState.datas.filter(
      (item: any) => item.mk_kode !== props.row.getValue('mk_kode')
    );
  ...
  >
    ...
    <div class="mt-4 overflow-x-auto">
      <table class="table border border-solid border-[#dddddd]">
        <thead>
          <tr>
            <th class="border border-solid border-[#dddddd]">Kode MK</th>
            <th class="border border-solid border-[#dddddd]">Nama</th>
            <th class="border border-solid border-[#dddddd]">SKS</th>
            <th class="border border-solid border-[#dddddd]">Semester</th>
          </tr>
        </thead>
        <tbody>
          <tr class="border border-solid border-[#dddddd]">
            <td class="border border-solid border-[#dddddd]">
              {{ row.getValue('mk_kode') }}
            </td>
            <td class="border border-solid border-[#dddddd]">
              {{ row.getValue('mk_nama') }}
            </td>
            <td class="border border-solid border-[#dddddd]">
              {{ row.getValue('mk_sks') }}
            </td>
            <td class="border border-solid border-[#dddddd]">
              {{ row.getValue('mk_semester') }}
            </td>
          </tr>
        </tbody>
      </table>
